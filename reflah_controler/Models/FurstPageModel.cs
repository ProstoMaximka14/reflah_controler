namespace reflah_controler.Models
{
    public class FurstPageModel
    {
        public string image_1 { get; set; }

        public string image_2 { get; set; }

        public string image_3 { get; set; }


        public string head_1_ru { get; set; }
        public string head_1_eng { get; set; }
        public string head_1_ger { get; set; }

        public string head_2_ru { get; set; }
        public string head_2_eng { get; set; }
        public string head_2_ger { get; set; }


        public string text_ru {  get; set; }
        public string text_eng { get; set; }
        public string text_ger { get; set; }


        public string block_1_ru { get; set; }
        public string block_1_eng { get; set; }
        public string block_1_ger { get; set; }

        public string block_2_ru { get; set; }
        public string block_2_eng { get; set; }
        public string block_2_ger { get; set; }

        public string block_3_ru { get; set; }
        public string block_3_eng { get; set; }
        public string block_3_ger { get; set; }

        public string block_4_ru { get; set; }
        public string block_4_eng { get; set; }
        public string block_4_ger { get; set; }

        public string block_5_ru { get; set; }
        public string block_5_eng { get; set; }
        public string block_5_ger { get; set; }

        public string block_6_ru { get; set; }
        public string block_6_eng { get; set; }
        public string block_6_ger { get; set; }

        public string block_1_title_ru { get; set; }
        public string block_1_title_eng { get; set; }
        public string block_1_title_ger { get; set; }

        public string block_2_title_ru { get; set; }
        public string block_2_title_eng { get; set; }
        public string block_2_title_ger { get; set; }

        public string block_3_title_ru { get; set; }
        public string block_3_title_eng { get; set; }
        public string block_3_title_ger { get; set; }

        public string block_4_title_ru { get; set; }
        public string block_4_title_eng { get; set; }
        public string block_4_title_ger { get; set; }

        public string block_5_title_ru { get; set; }
        public string block_5_title_eng { get; set; }
        public string block_5_title_ger { get; set; }

        public string block_6_title_ru { get; set; }
        public string block_6_title_eng { get; set; }
        public string block_6_title_ger { get; set; }

        public string image_4 { get; set; }
    }
}
